...
# surya-ocr-2 → ONNX (ONNX Runtime GenAI)

Convert [datalab-to/surya-ocr-2](https://huggingface.co/datalab-to/surya-ocr-2) — an OCR VLM on a
**Qwen3.5-VL** backbone (`model_type: qwen3_5`) — to three ONNX sub-parts (text decoder, vision
encoder, text embedding) with Olive, re-composed at runtime by ONNX Runtime GenAI.

Same hybrid GatedDeltaNet architecture as `Qwen-Qwen3.5-0.8B/builtin/`; **not** the old
`chandra/builtin` recipe (different architecture, Qwen3-VL-8B). `optimize.py` is fully
Python-driven — no static Olive JSON configs. See [HANDOFF.md](HANDOFF.md) for the full history:
the tokenizer/bos_token_id fixes, the real task prompts, and the traced `cuda_fp32` limitation.

## Build

```bash
python optimize.py --device cpu   --precision int4    # all three, cpu — BUILT + TESTED
python optimize.py --device cpu   --precision fp32    # BUILT + TESTED
...
(fp16 is GPU-only — Olive's ModelBuilder pass rejects it on CPU. `cuda_fp32` is deliberately
excluded — architecturally broken for this model, not just untested: genai's builder resolves
`(cuda, fp32)` to the packed-`Attention` op, which this model's attention override doesn't
support — `KeyError('root_input')`. Traced through genai's own builder source; see HANDOFF.md
§5.1). Builds from a local `./model` snapshot if present, else downloads
`datalab-to/surya-ocr-2`. Output → `<device>_<precision>/models/` (text.onnx, vision.onnx,
embedding.onnx, genai_config.json, processor_config.json, tokenizer files).

Windows: run with `PYTHONUTF8=1` (a vision-export print crashes on cp1252 otherwise).

## Inference and eval

`inference.py` drives one image through a built ONNX target using surya's own real task prompts
(sourced from datalab-to/surya's `surya/inference/prompts.py` — see `tasks.py`; don't invent
prompts for this model, a paraphrase can look right and still be wrong):

```bash
python inference.py --model_path cpu_int4/models --task ocr --image model/assets/corporate.png
python inference.py --model_path cpu_int4/models --task layout --image model/assets/form.png
```

`eval.py` batch-evaluates one or more built targets against the PyTorch checkpoint on real
document pages (validates `layout`/`table_rec` output against surya's actual JSON schema, and
reports text similarity vs. PyTorch for every task):

```bash
python eval.py --model model cpu_int4/models cpu_fp32/models   # explicit targets
python eval.py --model model                                    # auto-discovers built targets
```

Use the unsuffixed pages in `model/assets/` (`corporate.png`, `form.png`, `handwritten.png`, ...)
as input — the `*_layout/_text/_reading/_tablerec.png` siblings are this README's own
reference-**output** visualizations, not inputs.

## Verified end-to-end (2026-07-30)

Both `cpu_int4` and `cpu_fp32` built successfully and were tested with real inference across all
three tasks (`layout`, `table_rec`, `ocr`) against real document pages — 12/12 runs clean, valid
schema-conforming output, cross-checked against the PyTorch checkpoint. This is also the direct
regression test for the `KeyError('root_input')` failure this recipe was written to avoid.

Three runtime bugs were found and fixed along the way (see HANDOFF.md §4 and §7.4):
- `bos_token_id: null` crashing genai's config parser (this model has no real BOS token).
- A `WordLevel` tokenizer genai's C++ runtime can't load (re-encoded as BPE with an empty merge
  list — provably equivalent here since the pre-tokenizer already isolates single characters).
- A `U+FFFD` (replacement character) vocab entry that genai fails to index into its reverse
  lookup map, breaking decode of any generated token with that id — surfaces only on long
  free-form output (`ocr`, full-page HTML). Fixed with a Python-side fallback decoder
  (`inference.py::load_id_to_token`) used only when genai's own decode call fails.

Note: `--max_length` needs headroom for the image too — a single page can produce 2500–7000+ input
...
- [Olive](https://github.com/microsoft/Olive) from Olive

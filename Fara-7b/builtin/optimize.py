"""End-to-end optimization pipeline for microsoft/Fara-7B ONNX models.
...
def export_models(config_dir: str):
    ...
    for config in ("embedding.json",  "vision.json","text.json"): 
...
def update_genai_config(output_dir: str = MODELS_DIR, device: str = "gpu"):
    ...
    # Vision configuration (Fara-7B: patch_size=14, window_size=112)
    config["model"]["vision"] = {
    ...
    # Create processor_config.json (Fara-7B: patch_size=14, window_size=112, CLIP normalization)
    processor_config = {
        "processor": {
...
def main():
    parser = argparse.ArgumentParser(description="Optimize microsoft/Fara-7B ONNX models")

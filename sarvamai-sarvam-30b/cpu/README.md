# sarvamai/sarvam-30b — CPU optimization

This folder contains Olive recipes for optimizing sarvamai/sarvam-30b targeting the CPU EP.
...
- Example recipe filename: sarvamai-sarvam-30b_cpu_int4.json
...
3) Run Olive to build/optimize the model:
   ```
   olive run --config sarvamai-sarvam-30b_cpu_int4.json
   ```
...
- Due to the MoE structure, only ~2.4B parameters are active per forward pass, making
  INT4-quantized CPU inference significantly faster than the 30B total parameter count
  suggests.

---

This README was auto-generated for the CPU EP of sarvamai/sarvam-30b.

...
This example demonstrates how to convert [microsoft/Fara-7B](https://huggingface.co/microsoft/Fara-7B) vision-language model to ONNX format using Olive and run inference with ONNX Runtime GenAI.

Fara-7B is a fine-tuned vision-language model built on the Qwen2.5-VL-7B architecture (hidden_size=3584). The pipeline exports three sub-models (vision encoder, text embedding, text decoder), applies graph optimizations (Cast chain elimination, Gemm→MatMul conversion), and quantizes all three sub-models to INT4.
